    ...
    switch (args.level) {
        ...
        case 4:
            if (args.port == -1 || args.bind_addr == NULL) {
                fprintf(stdout, "err#Argument Parsing Failed\n");
                return -1;
            }
            break;
        case 2:
        ...
        case 3:
            ...
            break;
        default: fprintf(stdout, "err#Argument Parsing Failed\n"); return -1;
    }
...
int init_tcp_server(struct info *args) {
    ...
    if (socket_desc == -1) {
        fprintf(stdout, "err#Create Socket Failed\n: %s", strerror(errno));
        return -1;
    }
    ...
    if (setsockopt(socket_desc, SOL_SOCKET, SO_REUSEADDR, &tr, sizeof(int))
        != 0) {
        fprintf(stdout, "err#Set Socket Option Failed\n: %s", strerror(errno));
        return -1;
    }
...
int read_client_data(int client_sock, struct info *args) {
    ...
    if (read_size == -1) {
        //fprintf(stdout, "err#Data Receive Failed\n: %s", strerror(errno));
        return -1;
    }
...
int run_server(int socket_desc, struct info *args) {
    ...
    if (listen(socket_desc, 100) != 0) {
        //fprintf(stdout, "err#Listen to Port Failed\n: %s", strerror(errno));
        return -1;
    };
    fflush(stdout);
    while (1) {
        ...
        if (client_sock < 0) {
            //fprintf(stdout, "err#Accept Conn Failed: %s\n", strerror(errno));
            return -1;
        }

        char *ipAddr = inet_ntoa(((struct sockaddr_in *)&client)->sin_addr);
        fprintf(stdout, "trial#%s\n", ipAddr);

        if ((pid = fork()) < 0) {
            fprintf(stdout, "err#Fork Failed: %s\n", strerror(errno));
            return -1;
        } else if (pid == 0) {
            if (close(socket_desc) != 0) {
                //fprintf(stdout, "err#Close Listen Socket Failed: %s\n",
                //        strerror(errno));
                return -1;
            };

            if (read_client_data(client_sock, args) != 0) {
                //fprintf(stdout, "err#Read Client Data Failed: %s\n",
                //        strerror(errno));
                return -1;
            };

            if (close(client_sock) != 0) {
                //fprintf(stdout, "err#Close Client Socket Failed: %s\n",
                //        strerror(errno));
                return -1;
            };
            if (kill(getpid(), SIGKILL) != 0) {
                //fprintf(stdout, "err#Kill Child Failed: %s\n", strerror(errno));
                return -1;
            };
        }
        args->print_hint = 0;
...
void sendHint(const void *buffer, uintptr_t framep, struct info *args) {
    ...
    fprintf(stdout, "ans#buffer=0x%8X|ebp=0x%8X\n", addr, framep);
    fflush(stdout);
